...
class MemoryModule(tk.Frame):
    ...
    def setup_ui(self):
        form_frame = tk.LabelFrame(self, text=" DEPLOY EMERGENCY RESPONDER TEAM (MVT/MFT ALLOCATION) ", bg="#0f171e", fg="#ffcc00", font=("Courier", 10, "bold"))
        ...
        self.memory_type_box = ttk.Combobox(
            form_frame,
        ...
        deploy_btn = tk.Button(form_frame, text="Deploy Responder", bg="#1c2d3d", fg="white", font=("Courier", 9, "bold"), relief="flat", bd=0, command=self.add_memory_incident, width=22)
        deploy_btn.pack(side=tk.LEFT, padx=10)
        
        rand_mem_btn = tk.Button(form_frame, text="🎲 Randomize 4", bg="#1c2d3d", fg="white", font=("Courier", 9, "bold"), relief="flat", bd=0, command=self.randomize_mem_incidents)
        ...
        recall_btn = tk.Button(action_row, text="Recall Fleet / Responders", bg="#d35400", fg="white", font=("Courier", 9, "bold"), relief="flat", bd=0, command=self.terminate_memory_incident)
        recall_btn.pack(side=tk.LEFT, padx=5)
        
        compact_btn = tk.Button(action_row, text="⚡ Run Fleet Compaction ⚡", bg="#2980b9", fg="white", font=("Courier", 9, "bold"), relief="flat", bd=0, command=self.execute_fleet_compaction)
    ...
    def sync_mem_table_ui(self):
        ...
        for i in self.allocated_incidents_mem:
            if i.get("type") == "MFT":
                range_str = (
                    f"Partition {(i['start']//4)+1}"
                    f" | IF={i['internal_frag']}"
                )
            else:
                range_str = (
                    f"Blocks {i['start']:02d} - "
                    f"{i['start'] + i['size'] - 1:02d}"
                )
    ...
    def repaint_responder_grid(self):
        ...
        # Count how many free holes we have to calculate the total rows needed
        free_holes_count = 0
        i = 0
        ...
        # 🌟 DYNAMIC SCALING: Total rows = active rows + free hole rows
        total_rows = len(self.allocated_incidents_mem) + free_holes_count
        
        # Adjust height and vertical limits based on how crowded the chart is
        ...
        if not self.allocated_incidents_mem:
            ...
            # 1. Draw Active Allocations slanting up (Made height = 0.4 for smaller squares)
            for idx, item in enumerate(self.allocated_incidents_mem):
                width = item["size"]

                if item.get("type") == "MFT":
                    width = self.PARTITION_SIZE
            ...
            # 2. Draw Free Holes stacked above the allocations (Made height = 0.3 for a sleeker look)
            i = 0
            ...
            if memory_type == "MFT":
                ...
                if size > self.PARTITION_SIZE:
                    messagebox.showwarning(
                        "Allocation Failed",
                        f"Partition size is only {self.PARTITION_SIZE}"
                    )
                    return
                ...
                for i in range(self.NUM_PARTITIONS):
                    ...
                    for p in self.allocated_incidents_mem:
                        if p["start"] == partition_start:
                            occupied = True
                            break

                    if not occupied:
                        free_partition = partition_start
                        break

                if free_partition is None:
                    messagebox.showwarning(
                        "Memory Full",
                ...
                for i in range(
                    free_partition,
                    free_partition + self.PARTITION_SIZE
                ):
                    self.responder_fleet_map[i] = name

                internal_frag = self.PARTITION_SIZE - size

                nid = len(self.allocated_incidents_mem) + 1

                self.allocated_incidents_mem.append({
                    "id": nid,
                    "name": name,
                    "size": size,
                    "start": free_partition,
                    "internal_frag": internal_frag,
                    "type": "MFT"
                })

                self.mem_log_txt.insert(
                    tk.END,
                    f"[MFT] {name} allocated "
                    f"to Partition {(free_partition//4)+1} "
                    f"(IF={internal_frag})\n"
                )
    ...
    def execute_fleet_compaction(self):
        if self.memory_type_var.get() == "MFT":
            messagebox.showinfo(
                "MFT",
                "Compaction is not applicable to Fixed Partition Memory."
            )
            return
